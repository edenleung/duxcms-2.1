...
class Auth
{
    ...
    public static function init($config = [])
    {
        ...
        //不需要认证的模块，对后台认证有效
        self::$config['AUTH_LOGIN_NO'] = isset($config['AUTH_LOGIN_NO']) ? $config['AUTH_LOGIN_NO'] : ['index'=> ['login', 'verify'], 'common'=>'*'];
        //session前缀
        ...
        //数据库表和字段映射
        self::$config['AUTH_TABLE'] = isset($config['AUTH_TABLE']) ? $config['AUTH_TABLE'] : [
                                    'group'=> [
                                            'name' => 'group',
                                            'field'=> ['id'=>'id', 'power'=>'power_value'],
                                            ],
                                    'resource'=> [
                                            'name' => 'resource',
                                            'field'=> ['id'=>'id', 'pid'=>'pid', 'operate'=>'operate'],
                                            ],
                                ];
    }
    ...
    //权限认证检查
    public static function check($model, $config = [])
    {
        self::$model = $model;
        self::init($config);
    ...
    //自动获取模块信息
    public static function getModule($model, $config = [], $module_path = '', $module_suffix = '')
    {
        self::$model = $model;
        self::init($config);
        //完成数据库表名和字段名映射
